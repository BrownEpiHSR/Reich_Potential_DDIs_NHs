...
- dlist.&ddi.: List of generic drug names associated with a given DDI component.
               Includes the core_drug variable.
...
libname dclean   "your/library/path"; /* Cleaned DDI component Part D dispensing datasets */
libname dcln2    "your/library/path"; /* Cleaned DDI Component Part D dispensing datasets, v2 */
libname cmbfx    "your/library/path"; /* Cleaned DDI component datasets with fixed combination drugs */
libname lrenroll "your/library/path"; /* NH stays dataset with exclusions applied */
libname sum      "your/library/path"; /* DDI component datasets summary info, v2*/
libname names    "your/library/path"; /* Names of each DDI component */
libname clp2     "your/library/path"; /* Collapsed medication episode datasets, v2 */
libname nhtm2    "your/library/path"; /* Medication episodes during NH stays datasets */
...
data lrenroll.plevel_enroll;
	...
	if first.enroll_start; /* Keep the first record for each distinct bene_id_18900, enroll_start */
...
%macro combo_drug(ddi_sheet);
	...
	data &ddi._combo;
		...
		if combo_flag = 1 then do;
			core_drug = ''; /* Core_drug may not be the same as the active ingredient name we pull, so replacing with '' for now */
...
%macro create_drug_eps(ddi_sheet);
	...
	/*** Clean up the days' supply (DS) variable (hddays) ***/
	...
	/*** Limit dispensings to beneficiary's enrolled time by merging dataset with lrenroll.plevel_enroll ***/
		/* NOTE: We created the lrenroll.plevel_enroll dataset at the beginning of this program */
	...
	proc sql;
		...
		select distinct bene_id_18900, core_drug, hdl_gdname, episode, min_index_date, max_supply_enddt, count(*) as disp_number 
	    from &ddi._9
	    group by bene_id_18900, core_drug, episode;
