...
libname lrenroll "your/library/path"; /* Relevant NH time dataset with exclusions applied */
libname nhtm2    "your/library/path"; /* Medication episodes during NH time datasets */
libname merged   "your/library/path"; /* Merged medication episode datasets */
libname prev     "your/library/path"; /* Datasets with all beneficiaries to estimate prevalence for each DDI */
libname ddi      "your/library/path"; /* Datasets with lists of ddis and their components drug categories */
...
%macro flag_concurrent(component_1, component_2, ddi);
	...
	%else %if &ddi. = ddi20 %then %do;
		...
		run; /* Note that since nhtm2.&ddi_1._comb3 and nhtm2.&ddi_2._comb3 are identical, two data steps to delete records with core_drug = 'aspirin' isn't actually necessary */
	...
	/* If the ddi IS ddi30, delete any medication use episodes for diltiazem and verapamil from the CYP3A4-inhibitor dataset before merging */
		/* NOTE: We removed diltiazem and verapamil from the list of CYP3A4-inhibitors in ddi30 (calcium channel blocker + CYP3A4-inhibitor) 
	             because they are considered calcium channel blockers. */
...
/*** Import the excel file ***/
    ...
    proc import out=ddi.full_ddi_list (keep = ddi_paper ddi component_1 component_2 component_3 include_3a include_3b include_3c include_3d)
		datafile='your_path\DDI_List_To_SAS_v2.xlsx'
        dbms=xlsx
...
%macro import_class(component_1);
    ...
    proc import out=dlist.&ddi._cls (keep = core_drug class)
		datafile='your_path\DDIs_List_v5.xlsx'
        dbms=xlsx
        replace;  /* Replace existing dataset if it exists */
        sheet="&component_1.";
		getnames = yes;
...
/*** Create dataset with the list of DDIs and their corresponding DDI components to be run through the flag_concurrent_2 macro ***/
