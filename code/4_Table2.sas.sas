...
3. Create output for Table 2 
   (Top 12 Potential Drug-Drug Interactions Among Nursing Home Residents, 
   2018-2020 (N = 485,251 Residents)).

Programmer: Laura Reich   
 
Date Last Modified: April 22, 2025
*********************************************************************/
...
- ddi.ddi_list_3d: Dataset with the list of DDIs to be run through the estimate_prev and median_days macro 
...
- smcncr.calc_top12: Top 12 DDIs, prevalence and median days of DDI exposure
...
%macro estimate_prev(ddi);
	...
	proc freq data = prev.&ddi._prev2; 
...
/****************************************************************************************************/
/*** Run the median_days macro for the DDIs where at least one beneficiary had concurrent use  ***/
/****************************************************************************************************/
...
data _null_;
	...
	call execute(cats('%median_days(', ddi, ');'));
run;
...
/***************************************************/
/*** Limit dataset to the 12 most prevalent DDIs ***/
/***************************************************/
...
data smcncr.calc_top12;
	set smcncr.calc_all_2 (obs = 12);
run;
...
/*** Keep the relevant variables for Table 2 ***/

data smcncr.calc_table2_2;
	set smcncr.calc_top12;
...
title1 "TABLE 2: Prevalence and median days of exposure for top 12 DDIs";
